...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu":
        ...
        # Remove first line from explanation (contains ✅/❌ verdict — we show our own)
        exp_lines = explanation.strip().split("\n")
        exp_body = "\n".join(exp_lines[1:]).strip()
...
if __name__ == "__main__":
    main()
def get_question_keyboard(test_num, q_index):
    question = TESTS[test_num][q_index]
    total = len(TESTS[test_num])
...
def split_explanation(explanation):
    """Split explanation into English and Russian parts."""
    if "\n🇷🇺" in explanation:
        idx = explanation.index("\n🇷🇺")
        eng = explanation[:idx].strip()
        rus = explanation[idx+1:].strip()  # keeps 🇷🇺 prefix
        return eng, rus
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Test selection from /start menu
    if data.startswith("select_test_"):
        test_num = int(data.split("_")[-1])
        await send_question(query, context, test_num, 0)

    # Show Russian translation of question (hide button after)
    ...
    # Answer selected
        ...
        eng_explanation, rus_explanation = split_explanation(explanation)

        if chosen == correct:
            result_text = (
                f"✅ *Правильно!*\n"
                f"Правильный ответ: {question['options'][correct]}\n\n"
                f"📖 {eng_explanation}"
            )
        else:
            result_text = (
        ...
        keyboard = []
        if rus_explanation:
            keyboard.append([InlineKeyboardButton("🇷🇺 Показать перевод", callback_data=f"show_expl_tr_{test_num}_{q_index}_{chosen}")])
        keyboard.append([InlineKeyboardButton("➡️ Следующий вопрос", callback_data=f"next_{test_num}_{q_index + 1}")])
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text(result_text, parse_mode="Markdown", reply_markup=reply_markup)

    # Show Russian translation of explanation (hide button after)
...
async def send_question(query, context, test_num, q_index):
    ...
    reply_markup, _ = get_question_keyboard(test_num, q_index)
    await query.edit_message_text(text, parse_mode="Markdown", reply_markup=reply_markup)
