...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu":
        ...
        # Extract Russian part from explanation for spoiler
        exp_lines = explanation.strip().split("\n")
        ru_lines = [l for l in exp_lines if "\U0001f1f7\U0001f1fa" in l or "🇷🇺" in l]
        ru_text = " ".join(ru_lines).replace("\U0001f1f7\U0001f1fa *Перевод:*", "").replace("🇷🇺", "🇷🇺").strip()
        ...
        if chosen == correct:
            ...
            result_text = (
                ...
                f"<tg-spoiler>{ru_text}</tg-spoiler>"
            )
        else:
            result_text = (
                f"❌ <b>Неверно!</b>\n"
        ...
        if next_index < total:
            ...
            passed = "\U0001f389 <b>СДАЛ(А)!</b>" if score >= 18 else "\U0001f614 <b>Не сдал(а). Попробуй ещё раз!</b>"
            keyboard = [[InlineKeyboardButton("\U0001f3e0 Главное меню", callback_data="menu")]]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text(
                f"{result_text}\n\n"
                f"━━━━━━━━━━━━━━━\n"
                f"\U0001f4ca <b>Результат: {score} из {total}</b>\n"
                f"{passed}\n"
                f"<i>(Для сдачи нужно 18 из 24)</i>",
                reply_markup=reply_markup,
                parse_mode="HTML"
            )
...
async def send_question(query, context):
    ...
    q_text = question['question'].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    t_text = question['translation'].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    opts_html = "\n".join(
        [f"<b>{chr(65+i)})</b> {opt}" for i, opt in enumerate(question["options"])]
    )
    await query.edit_message_text(
        f"\U0001f4dd <b>Вопрос {q_index + 1} из {total}</b> — Тест {test_num}\n\n"
        f"<b>{q_text}</b>\n"
        f"<tg-spoiler>{t_text}</tg-spoiler>\n\n"
        f"{opts_html}",
        reply_markup=reply_markup,
        parse_mode="HTML"
    )
