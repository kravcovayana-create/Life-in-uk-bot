...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "👋 Добро пожаловать в бот *Life in the UK*!\n\nВыберите тест:",
        parse_mode="Markdown",
...
async def send_question(query, context):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🇷🇺 Показать перевод", callback_data="show_q_tr")],
    ]

    await query.edit_message_text(
        f"📝 *Вопрос {q_index + 1} из {total}* — Тест {test_num}\n\n"
        f"*{question['question']}*\n\n"
        f"{options_text}",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode="Markdown"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu":
        ...
        keyboard = []
        if rus_part:
            keyboard.append([InlineKeyboardButton("🇷🇺 Показать перевод", callback_data="show_ans_tr")])
