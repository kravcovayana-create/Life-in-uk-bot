...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu":
        ...
        # Show Russian translation of question, remove button
        ...
        # Show Russian translation of answer, remove button
        test_num = context.user_data["test"]
        ...
        keyboard = []
        if rus_part:
            keyboard.append([InlineKeyboardButton("\U0001f1f7\U0001f1fa Показать перевод", callback_data="show_ans_tr")])

        if next_index < total:
            keyboard.append([InlineKeyboardButton("➡️ Следующий вопрос", callback_data="next")])
            await query.edit_message_text(result_text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        else:
            score = context.user_data["score"]
            passed = "\U0001f389 *СДАЛ(А)!*" if score >= 18 else "\U0001f614 *Не сдал(а). Попробуй ещё раз!*"
            keyboard.append([InlineKeyboardButton("\U0001f3e0 Главное меню", callback_data="menu")])
            await query.edit_message_text(
                f"{result_text}\n\n━━━━━━━━━━━━━━━\n"
                f"\U0001f4ca *Результат: {score} из {total}*\n{passed}\n_(Для сдачи нужно 18 из 24)_",
                reply_markup=InlineKeyboardMarkup(keyboard),
                parse_mode="Markdown"
            )
...
def split_explanation(explanation):
    """Split explanation: English part shown immediately, Russian 🇷🇺 part hidden behind button."""
    lines = explanation.strip().split("\n")
    eng_lines = []
    rus_lines = []
    for line in lines:
        stripped = line.strip()
        if stripped.startswith("🇷🇺") or stripped.startswith("\U0001f1f7"):
            rus_lines.append(stripped)
        else:
...
async def send_question(query, context):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("\U0001f1f7\U0001f1fa Показать перевод", callback_data="show_q_tr")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
